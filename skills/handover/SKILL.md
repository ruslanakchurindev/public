...
`latest` resolves the newest artifact produced in the current worktree; with none there it falls back to the repo's newest and warns on stderr, naming the worktree that produced it and whether that directory still exists. Both steps consider every artifact, named or not, so `latest` and `list` never disagree. Use `--name NAME` only when several workstreams in one repo need separate threads; being an explicit selector, it skips the worktree preference. Always set `HANDOVER_MODEL_NAME` to your model identifier — there is no auto-detection. On a machine holding several GitHub accounts, also set `HANDOVER_GH_ALIAS` to the alias this repository requires, so the snapshot's PR lookup does not run as whichever account is globally active. Full rules, store layout and environment variables: [README.md](README.md#configuration).
...
4. Capture verification reproducibly: exact command, working directory, prerequisites (env vars, services, ports), and result — passed, failed with key detail, or skipped and why. Fold unfinished todos into Next steps, and note blockers honestly: missing credentials, approvals, failing tests, uncertain requirements, external state.
5. Before writing, confirm every irrecoverable item is in — anything reconstructible from Git, tests, command history, or source is padding, and the irrecoverable belongs first. Write the artifact (Output format below) to a temp file, then save it — the body is read from stdin: `HANDOVER_MODEL_NAME="<your-model>" scripts/handover.sh save <repo-path> < /tmp/handover.md`. Reply with the saved path plus a 3–5 line summary, not the full artifact, unless the user asks for it.

## Resume workflow

1. Locate the artifact: the path the user gave, else `scripts/handover.sh latest .` (or `--name NAME` when the user names a thread). When the request names a topic rather than a path, run `scripts/handover.sh list .`, read each candidate's title line, and offer a shortlist — do not guess. If none exists, say so and fall back to normal discovery; do not invent a prior session.
2. Read it. Compare the `Generated / Repo / Workspace / Model / Branch / HEAD / Dirty` stamp against the current workspace (re-run the snapshot script). If anything drifted, say exactly what, and re-verify the Workspace state and Verification sections before trusting them; the conversation-state sections (Conversation state that matters, Decisions, Failed approaches, Do NOT) remain valid. All worktrees of a repo share one store, so a loaded handover may come from a different worktree: `latest` prefers this worktree's own and warns on stderr when it fell back — surface that warning and confirm whether to switch, since branch, HEAD, and uncommitted work are per-worktree. Given an explicit path instead, check the metadata's `worktree` field (or `workspace-path` on older artifacts).
3. Honor Do NOT entries and do not retry Failed approaches without new information.
4. Summarize the objective, current state, drift, and recommended first action, then wait. Start work from "Resume here" only when the request explicitly said to start or continue; confirm the objective first if the workspace drifted or the artifact is ambiguous.
...
Omit genuinely empty sections rather than padding them. "Conversation state that matters" appears only when the session's value was reasoning rather than code — translation, explanation, planning, review reasoning, stakeholder alignment, "what should I tell X" work — and then carries the user's real concern, the framing accepted vs. rejected and why, and any wording or tone to preserve. A filled artifact: [EXAMPLES.md](EXAMPLES.md).

## Quality bar
- Prefer concrete paths, commands, branches, shas, ports, URLs, and exact error messages over general summaries.
- Use repository-relative paths for repo files; reserve absolute paths for machine-local state (temp files, untracked scratch) and label them as such.
- Every command and path must have actually been run or seen this session — never invent.
